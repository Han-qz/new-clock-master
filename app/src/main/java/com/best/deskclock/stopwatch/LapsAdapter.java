 ...
 * 按相反顺序显示圈数列表。也就是说，最新的一圈在顶部，最老的一圈位于底部。
 */
class LapsAdapter extends RecyclerView.Adapter<LapsAdapter.LapItemHolder> {
     ...
     * @param totalTime time accumulated for the current lap and all prior laps 当前圈和之前所有圈的累积时间
     */
    void updateCurrentLap(RecyclerView rv, long totalTime) {
        // If no laps exist there is nothing to do.如果不存在重叠，则无需执行任何操作。
        if (getItemCount() == 0) {
            return;
        }
        ...
        if (currentLapView != null) {
            // Compute the lap time using the total time. 使用总时间计算圈时间
    ...
    /**
     * Record a new lap and update this adapter to include it. 记录新的一圈并更新此适配器以将其包括在内。
     *
     * @return a newly cleared lap返回新清除的圈
     */
    Lap addLap() {
        ...
        if (getItemCount() == 10) {
            // 10 total laps indicates all items switch from 1 to 2 digit lap numbers. 总共10圈表示所有项目从1位圈数切换到2位圈数。
            notifyDataSetChanged();
        } else {
            // New current lap now exists. 新的当前圈现在存在。
            notifyItemInserted(0);

            // Prior current lap must be refreshed once with the true values in place. 上一个当前圈必须使用正确的值刷新一次。
            notifyItemChanged(1);
        }
    ...
    /**
     * Remove all recorded laps and update this adapter. 删除所有记录的圈数并更新此适配器。
     */
    void clearLaps() {
